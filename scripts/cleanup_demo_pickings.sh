...
# origin looks like a demo purchase order (P...) and whose id is above
# the known baseline.
#
...
echo "[cleanup] purging demo completed pickings (id > $BASELINE_ID, origin LIKE 'P%', state='done') from $DB ..."

env PGPASSWORD=odoo psql -U odoo -h localhost -d "$DB" <<'SQL'
DELETE FROM stock_move_line
WHERE picking_id IN (
    SELECT id FROM stock_picking
    WHERE id > 7 AND origin LIKE 'P%' AND state = 'done'
);
